...
**功能描述**: 用于创建新任务，支持设置任务的基本信息、优先级、时间安排等。
...
```json
{
  ...
  "tags": "array|null",         // 【可选】任务标签列表，最多10个标签
  "service_ids": "array|string|null",   // 【可选】关联服务ID列表，占位字段
  "due_date": "string|null",           // 【可选】任务截止日期（2025-11-16）
  "planned_start_time": "string|null", // 【可选】计划开始时间（ISO 8601格式UTC）
  "planned_end_time": "string|null"    // 【可选】计划结束时间（ISO 8601格式UTC）
}
```
...
- `due_date`: 【可选】日期格式2025-11-16
- `planned_start_time`: 【可选】ISO 8601格式的时间时间，UTC 时间2024-11-15T18:00:00Z
- `planned_end_time`: 【可选】ISO 8601格式的时间时间，UTC 时间2024-11-15T18:00:00Z
...
**功能描述**:获取当前用户所有任务的标签列表，用于标签选择和管理。
...
**返回字段说明**:
- `task`: 完成后的任务基本信息
- `reward`: 奖励信息，iPhone 17 Pro

---

## 7. 【番茄】开启专注状态（看 Swagger 文档）
