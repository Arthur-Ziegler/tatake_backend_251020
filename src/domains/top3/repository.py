...
class Top3Repository:
    ...
    def get_by_user_and_date(self, user_id: UUID, target_date: date) -> Optional[TaskTop3]:
        """根据用户和日期获取Top3"""
    ...
    def is_task_in_today_top3(self, user_id: UUID, task_id: UUID) -> bool:
        """检查任务是否在今天的Top3中"""
        today = date.today()
        ...
        if task_ids and isinstance(task_ids[0], dict):
            ...
            return any(item.get('task_id') == str(task_id) for item in task_ids)
        else:
            # 旧格式：['uuid']
            return str(task_id) in task_ids
