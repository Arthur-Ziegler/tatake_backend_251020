...
提供统一的数据访问层抽象，实现Repository模式的核心功能。
支持泛型操作，确保类型安全和代码复用。

功能特性：
1. 基础CRUD操作（创建、读取、更新、删除）
2. 查询方法（获取所有、存在性检查、计数）
3. 参数验证和异常处理
4. 泛型类型支持，确保类型安全
...
使用示例：
    ...
    >>>
    >>> # 更新用户
    >>> updated_user = user_repo.update(user.id, {"nickname": "李四"})
    >>>
    >>> # 检查存在性和计数
    >>> exists = user_repo.exists(email="zhangsan@example.com")
...
from sqlmodel import Session, SQLModel, select, func, and_, or_
...
class BaseRepository(Generic[T]):
    ...
    提供统一的数据访问接口，支持泛型操作和类型安全。
    所有具体Repository类都应该继承自这个基类。
    ...
    def get_by_id(self, obj_id: Union[str, UUID]) -> Optional[T]:
        ...
        Raises:
            RepositoryValidationError: ID参数无效时
            RepositoryError: 数据库操作错误时
        """
        try:
            # 参数验证
            if obj_id is None:
                raise RepositoryValidationError("obj_id参数不能为None")
    ...
    def update(self, obj_id: Union[str, UUID], update_data: Dict[str, Any]) -> Optional[T]:
        ...
        Raises:
            RepositoryValidationError: 数据验证失败时
            RepositoryNotFoundError: 对象不存在时
            RepositoryError: 数据库操作错误时
        """
        try:
            # 参数验证
            ...
            if not update_data:
                raise RepositoryValidationError("update_data参数不能为空字典")
    ...
    def exists(self, **filters) -> bool:
        ...
        try:
            if not filters:
                raise RepositoryValidationError("至少需要提供一个过滤条件")
            ...
            if not conditions:
                raise RepositoryValidationError("提供的过滤条件中没有有效的字段")

            statement = statement.where(and_(*conditions))

            # 执行查询并检查结果
            count = self.session.exec(statement).scalar()
