...
class TestServiceFactory:
    ...
    async def test_initialization(self, mock_factory):
        ...
        with patch('api.dependencies.create_async_engine') as mock_engine, \
             ...
             patch('api.dependencies.redis.from_url') as mock_redis:
    ...
    def test_get_redis_client(self, mock_factory):
        """测试Redis客户端获取"""
        # 模拟Redis客户端
        mock_redis = AsyncMock()
        mock_factory._redis_client = mock_redis

        # 获取Redis客户端
        redis_client = mock_factory.get_redis_client()
        assert redis_client == mock_redis

    def test_get_redis_client_uninitialized(self, mock_factory):
        """测试未初始化时获取Redis客户端"""
        with pytest.raises(RuntimeError, match="ServiceFactory未初始化"):
            mock_factory.get_redis_client()
    ...
    async def test_close(self, mock_factory):
        ...
        mock_factory._redis_client = mock_redis
        mock_factory._database_manager = mock_db_manager
...
class TestFastAPIDependencies:
    ...
    async def test_get_redis_client(self):
        ...
        with patch('api.dependencies.service_factory') as mock_factory:
            mock_redis = AsyncMock()
            mock_factory.get_redis_client.return_value = mock_redis
...
class TestDependencyIntegration:
    ...
    async def test_service_factory_end_to_end(self):
        ...
        with patch('api.dependencies.create_async_engine') as mock_engine, \
             patch('api.dependencies.sessionmaker') as mock_sessionmaker, \
             patch('api.dependencies.redis.from_url') as mock_redis, \
             patch('api.dependencies.DatabaseManager') as mock_db_manager:
            ...
            mock_redis.return_value = AsyncMock()
            mock_db_manager_instance = AsyncMock()
            ...
            # 验证可以获取各种Repository
            async with factory.get_database_session() as session:
                user_repo = factory.get_user_repository(session)
                task_repo = factory.get_task_repository(session)
                focus_repo = factory.get_focus_repository(session)
                reward_repo = factory.get_reward_repository(session)
                chat_repo = factory.get_chat_repository(session)

                assert user_repo is not None
                assert task_repo is not None
                assert focus_repo is not None
                assert reward_repo is not None
                assert chat_repo is not None

            # 验证可以获取各种Service
            async with factory.get_database_session() as session:
                auth_service = await factory.get_auth_service(session)
                user_service = await factory.get_user_service(session)
                task_service = await factory.get_task_service(session)
