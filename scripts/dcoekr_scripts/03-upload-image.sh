...
check_export_file() {
    ...
    local export_file="${output_dir}/supertool_deploy_${DOCKER_VERSION}.tar.gz"

    # 先尝试读取临时文件中的路径
    if [[ -f "/tmp/supertool_last_export.txt" ]]; then
        export_file=$(cat "/tmp/supertool_last_export.txt")
    fi
...
upload_image() {
    ...
    local remote_file="${remote_dir}/supertool_deploy_${DOCKER_VERSION}.tar.gz"
...
main() {
    ...
    if [[ -f "/tmp/supertool_last_export.txt" ]]; then
        ...
        temp_file_path=$(cat "/tmp/supertool_last_export.txt")
